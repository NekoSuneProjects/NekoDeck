...
import { Bot, Box, Copy, Download, ExternalLink, FileJson2, MessageSquare, Package, Plus, RefreshCw, Save, Settings2, ShieldCheck, Trash2, Upload, X } from 'lucide-react';
...
const blank = () => ({
  ...
  rootapp: { projectId: '', version: '0.1.0', uploadHost: '', settings: '{}', permissions: '{}' },
...
export default function BotBuilder() {
  ...
  const choose = (bot) => {
    ...
    setForm({
      ...
      rootapp: { projectId: bot.config?.rootapp?.projectId || '', version: bot.config?.rootapp?.version || '0.1.0', uploadHost: bot.config?.rootapp?.uploadHost || '', settings: pretty(bot.config?.rootapp?.settings), permissions: pretty(bot.config?.rootapp?.permissions) },
  ...
  const buildRootPkg = async () => {
    if (!selectedId) return setMessage('Save the bot first.');
    setPkgBusy(true); setMessage('Installing Root Bot dependencies, compiling and building rootapp.pkg…');
    try {
      const response = await fetch(`/api/bots/${selectedId}/export/root-pkg`, { headers: headers() });
      if (!response.ok) { const data = await response.json().catch(() => ({})); throw new Error(data.error || `HTTP ${response.status}`); }
      const blob = await response.blob(); const disposition = response.headers.get('content-disposition') || '';
      const filename = disposition.match(/filename="([^"]+)"/)?.[1] || 'rootapp.pkg';
      saveBlob(blob, filename); setMessage(`Root Bot package built successfully: ${filename}`);
    } catch (error) { setMessage(error.message); }
    finally { setPkgBusy(false); }
  };
  const publishRoot = async () => {
    if (!selectedId) return setMessage('Save the bot first.');
    setPkgBusy(true); setMessage('Building Root Bot package and uploading it to Root…');
    try {
      const data = await api(`/api/bots/${selectedId}/publish/root`, { method: 'POST', body: '{}' });
      setMessage(data.message || 'Root Bot uploaded successfully.');
    } catch (error) { setMessage(error.message); }
    finally { setPkgBusy(false); }
  };
  ...
  return <>
    ...
    {open && <div className="bb-backdrop" onMouseDown={e => e.target === e.currentTarget && setOpen(false)}><div className="bb-window">
      <header><div><span>NekoDeck v0.7</span><h2>Bot Builder</h2><p>Build Discord.js bots and Root community chatbots from one command model.</p></div><button onClick={() => setOpen(false)}><X/></button></header>
      ...
      <main>
        ...
        {tab === 'export' && <div className="bb-export"><section><div className="bb-export-icon discord">D</div><h3>Discord.js source</h3><p>Node.js 24.17+ project with slash-command registration, bot runtime and optional auto replies.</p><button disabled={!selectedId || !form.platforms.discord} onClick={() => download('discord')}><Download size={14}/>Download Discord Bot ZIP</button></section><section><div className="bb-export-icon root">R</div><h3>Root Bot</h3><p>Download editable TypeScript source, or let NekoDeck compile it and return a ready Root <code>.pkg</code>.</p><div className="bb-export-buttons"><button disabled={!selectedId || !form.platforms.rootapp || pkgBusy} onClick={() => download('root')}><Download size={14}/>Source ZIP</button><button disabled={!selectedId || !form.platforms.rootapp || pkgBusy} onClick={buildRootPkg}><Package size={14}/>{pkgBusy ? 'Building…' : 'Build .pkg'}</button><button disabled={!selectedId || !form.platforms.rootapp || pkgBusy || !selected?.credentialStatus?.rootBotAuthToken} onClick={publishRoot}><Upload size={14}/>Build + Upload</button><button disabled={!selectedId || !form.platforms.rootapp || pkgBusy} onClick={showManifest}><FileJson2 size={14}/>Manifest</button></div></section>{manifest && <section className="bb-manifest"><header><h3>root-manifest.json</h3><button onClick={() => navigator.clipboard?.writeText(pretty(manifest))}><Copy size={13}/>Copy</button></header><pre>{pretty(manifest)}</pre></section>}</div>}
      </main>
    </div></div>}
  </>;
}
