...
function normalizeUploadHost(value) {
...
function publicRootConfig(instance) {
  ...
  return {
    ...
    uploadHost: normalizeUploadHost(root.uploadHost || ''),
    clientDeploy: 'client/dist',
    serverLaunch: 'server/dist/main.js',
    serverDeploy: ['server/dist'],
    serverNodeModules: ['server/node_modules']
...
function rootCommands(root) {
  const hostArg = root.uploadHost ? ` --host ${root.uploadHost}` : '';
  return {
    build: 'npm install && npm run build',
    package: 'npx rootsdk build package --output-file ./rootapp.pkg --project-folder .',
    upload: `npx rootsdk upload package --file ./rootapp.pkg --authToken $ROOT_AUTH_TOKEN${hostArg}`
  };
}
...
function registerRootAppRoutes(app, store, options = {}) {
  ...
  app.get('/api/rootapp/:id', (req, res) => {
    ...
    res.json({
      ...
      commands: root.projectId && isSemver(root.version) ? rootCommands(root) : null
    });
  });
  ...
  app.put('/api/rootapp/:id', requireAdmin, (req, res) => {
    ...
    try {
      ...
      if (req.body?.uploadHost !== undefined) next.uploadHost = normalizeUploadHost(req.body.uploadHost);
