...
function createStore(dataDir) {
  ensureDir(dataDir); const dbPath = path.join(dataDir, 'nekodeck.json'); const key = loadMasterKey(dataDir);
  const initial = { version:1, instances:[], settings:{ accent:'#45f58c', density:'comfortable', glass:true } };
  function read(){ if(!fs.existsSync(dbPath)){fs.writeFileSync(dbPath,JSON.stringify(initial,null,2));return structuredClone(initial)} try{return {...initial,...JSON.parse(fs.readFileSync(dbPath,'utf8'))}}catch{return structuredClone(initial)} }
  function write(db){const tmp=`${dbPath}.tmp`;fs.writeFileSync(tmp,JSON.stringify(db,null,2));fs.renameSync(tmp,dbPath)}
  function publicInstance(item){const c=item.secrets?decryptJson(item.secrets,key):{};return {id:item.id,templateId:item.templateId,name:item.name,createdAt:item.createdAt,updatedAt:item.updatedAt,config:item.config||{},credentialStatus:{discordClientId:Boolean(c.discordClientId),discordClientSecret:Boolean(c.discordClientSecret),botToken:Boolean(c.botToken),providerApiKey:Boolean(c.providerApiKey),providerClientId:Boolean(c.providerClientId),providerClientSecret:Boolean(c.providerClientSecret),providerSession:Boolean(c.providerSession),providerConnected:Boolean(c.refreshToken||c.accessToken)},discordClientId:c.discordClientId||null}}
  return {
    ...
    getPrivateInstance(id){const i=read().instances.find(x=>x.id===id);return i?{...i,credentials:i.secrets?decryptJson(i.secrets,key):{}}:null},
    ...
    mergeCredentials(id,patch){const db=read(),i=db.instances.find(x=>x.id===id);if(!i)return null;const current=i.secrets?decryptJson(i.secrets,key):{};i.secrets=encryptJson({...current,...(patch||{})},key);i.updatedAt=new Date().toISOString();write(db);return publicInstance(i)},
